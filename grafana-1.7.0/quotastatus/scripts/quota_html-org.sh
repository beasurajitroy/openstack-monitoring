...
compute=`nova limits --tenant ${tenantid} | grep -e 'Cores\|RAM'`
coresused=`echo $compute | awk -F'|' '{print $3}'`
coresmax=`echo $compute | awk -F'|' '{print $4}'`
ramused=`echo $compute | awk -F'|' '{print $7}'`
rammax=`echo $compute | awk -F'|' '{print $8}'`
